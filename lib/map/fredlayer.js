define(['leaflet', 'rbush', 'helper', 'moment'],
  function (L, rbush, helper) {
    ...
    var nodeRadius = config.icon.fred.base.radius;
    ...
    function prepareLabel(fillStyle, fontSize, offset, stroke) {
      return function (d) {
        ...
        // console.log('prep label', d);
        return {
          position: L.latLng(d.calculated_latitude, d.calculated_longitude),
          label: d.last_ssid + ' (' + d.bssid + ')',
          offset: offset,
          fillStyle: fillStyle,
          height: fontSize * 1.2,
          font: font,
          stroke: stroke,
          width: measureText(font, d.last_ssid + ' (' + d.bssid + ')').width
        };
      };
    }
    ...
    function mkMarker(dict, iconFunc) {
      return function (d) {
        // console.log('mkMarker', d);
        var m = L.circleMarker([d.calculated_latitude, d.calculated_longitude], iconFunc(d));
        m.setRadius(nodeRadius);
        ...
        */
        m.bindTooltip(helper.escape(d.last_ssid + '(' + d.bssid + ')'));
    ...
    function getIcon() {
      return Object.assign({}, config.icon.fred.base);  // currently only assign base attributes
    }
    ...
    return L.GridLayer.extend({
      ...
      setData: function (data, map) {
        var iconOnline = getIcon();
        // Check if init or data is already set
        if (groupBase) {
          groupBase.clearLayers();
        }

        var nodesOnline = data;
        var dataDict = {};
        ...
        var markersOnline = nodesOnline.map(mkMarker(dataDict, function () {
          return iconOnline;
        }));

        groupBase = L.featureGroup(markersOnline).addTo(map);
      ...
      hide: function (map) {
        this.isHidden = true;
        map.removeLayer(groupBase);
      },
      show: function (map) {
        this.isHidden = false;
        map.addLayer(groupBase);
      },
      ...
      prepareLabels: function () {
        ...
        var labelsOnline = d.map(prepareLabel(null, 11, 8, true));
        var labels = []
          .concat(labelsOnline);
      ...
      createTile: function (tilePoint) {
        // console.log('labellayer create tile');
