    ...
    return function () {
      ...
      function update() {
        return Promise.all(config.dataPath.map(helper.getJSON))
          .then(handleData)
          .then(function (data) {
            return Promise.all(config.fred.dataPath.map(helper.getJSON)).then(function (response) {
              data.fred = response;
              return data;
            });
          });
      ...
      setTimeout(
        function () {
          var fredToggle = document.createElement('div');
          fredToggle.classList.add('fred-toggle');
          fredToggle.innerHTML = '<input id="fred-toggle" type="checkbox" checked="checked" /> <label for="fred-toggle">Fred Daten</label>';
          document.querySelector('.leaflet-control-layers-toggle').appendChild(fredToggle);
        }, 1000
      );
    };
  });
