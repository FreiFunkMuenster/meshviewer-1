define(['map/clientlayer', 'map/labellayer', 'map/button', 'leaflet', 'map/activearea', 'map/fredlayer'],
  function (ClientLayer, LabelLayer, Button, L, ActiveArea, FredLayer) {
    'use strict';
    ...
    return function (linkScale, sidebar, buttons) {
      ...
      var fredLayer = new FredLayer({ minZoom: config.clientZoom });
      // fredLayer.addTo(map);
      fredLayer.setZIndex(5);
      ...
      map.on('zoom', function () {
        ...
        console.log('hasLayer', map.hasLayer(fredLayer));
        console.log('map', map);

        if (map.getZoom() < config.fred.minZoom) {
          fredLayer.hide(map);
        } else {
          fredLayer.show(map);
          fredLayer.redraw();
      ...
      map.on('baselayerchange', function (e) {
        ...
        fredLayer.options.maxZoom = map.options.maxZoom;
      ...
      self.setData = function setData(data) {
        console.log('map setting data');
        nodeDict = {};
        linkDict = {};

        clientLayer.setData(data);
        labelLayer.setData(data, map, nodeDict, linkDict, linkScale);
        console.log(fredLayer);
        console.log(data);
        fredLayer.setData(data.fred[0], map);
